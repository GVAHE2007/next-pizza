...
import { DashboardIngredientSchema, Mode } from "@/@types/admin";
...
export default function DashboardIngredientPage() {
    ...
    const onSubmit = async (data: DashboardIngredientSchema) => {
        console.dir(data)
        const imgUrl = await uploadImage(data.imgUrl as File)
        try {
            if (mode.mode === "create") {
                const create = await ingredientAdmin({ ...data, imgUrl: imgUrl as string }, "create")
                if (create) {
                ...
                await ingredientAdmin({ ...data, imgUrl: imgUrl as string }, "update", mode.id)
                toast.success("Ingredient updated")
            }
            else if (mode.mode === "delete" && mode.id) {
                const countent = await ingredientAdmin({ ...data, imgUrl: imgUrl as string }, "delete", mode.id)
                if (countent) {
            ...
            <div>
                ...
                <FormProvider {...form}>
                    <form className="grid justify-normal justify-items-start gap-5" onSubmit={form.handleSubmit(onSubmit)}>
                        ...
                        <ImageViewer form={form as any} mode={mode} previewUrl={previewUrl} setPreviewUrl={setPreviewUrl} />
