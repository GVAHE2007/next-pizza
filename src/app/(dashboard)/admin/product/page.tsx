...
import { DashboardProductSchema, Mode } from "@/@types/admin";
...
export default function DashboardProductPage() {
    ...
    const onSubmit = async (data: DashboardProductSchema) => {
        const selectedIngredients = Object.entries(data.ingredients || {})
            .filter(([_, value]) => value)
            .map(([id]) => Number(id));
        console.dir(data)
        const imgUrl = await uploadImage(data.imgUrl as File) as string
        try {
            if (mode.mode === "create") {
